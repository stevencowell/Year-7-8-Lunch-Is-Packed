(() => {
  ...
  if (homeProgress) {
    const responses = Object.keys(localStorage).filter(k => /^lunchpacked:m\d+-/.test(k) && (localStorage.getItem(k)||'').trim().length >= 20).length;
    const checked = Object.keys(localStorage).filter(k => k.startsWith('lunchpacked:check:')).filter(k => { try { return JSON.parse(localStorage.getItem(k) || '{}').correct; } catch (_) { return false; } }).length;
    const activities = new Set(Object.keys(localStorage).filter(k => k.startsWith('lunchpacked:activity:') && (localStorage.getItem(k)||'').trim().length >= 20).map(k => k.match(/:a(\d+)-/)?.[1]).filter(Boolean)).size;
